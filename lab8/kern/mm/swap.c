...
{
     struct mm_struct *least_pgfault = mm;//最少缺页次数的进程mm块
     list_entry_t *le, *last; //用于遍历链表
     ...
     do { //遍历进程链表
            if ((le = list_next(le)) != &proc_list) {
                ...
                if(next->pgfault_num<min && (next->is_user==1)){ //找出缺页次数最小的用户进程
                    list_entry_t *head = (list_entry_t*) (next->mm)->sm_priv;//检查是否有物理页可换
                    list_entry_t *le = head->next;
                    if (head==le) continue; //说明链表为空，无物理页可换
                     least_pgfault = next->mm;
                     min = next->pgfault_num;
                }
            }
     } while (le != last);
     ...
     for (i = 0; i != n; ++ i)
     {
          ...
          int r = sm->swap_out_victim(least_pgfault, &page, in_tick);
          if (r != 0) {
